...
export default function Template3Preview({ content }) {
  ...
  const calculateTimeLeft = useCallback(() => {
    const now = new Date();
    const weddingTime = new Date(data.weddingDate);
          ...
          <div className='max-w-4xl mx-auto relative z-10'>
            ...
            {submitSuccess ? (
              <motion.div
                ...
                className='bg-gradient-to-br from-amber-50 via-pink-50 to-rose-50 rounded-3xl shadow-2xl p-10 md:p-14 text-center border border-amber-100/50'
              >
                <motion.div
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  transition={{ delay: 0.2, type: "spring", stiffness: 200 }}
                  className='w-24 h-24 mx-auto mb-6 rounded-full bg-gradient-to-br from-amber-400 to-pink-400 flex items-center justify-center'
                >
                ...
                <div className='flex justify-center gap-2'>
                  {[...Array(5)].map((_, i) => (
                    ...
                    >
                      <Flower className='w-6 h-6 text-amber-500' />
                    </motion.div>
                  ))}
                </div>
              </motion.div>
                ...
                <form onSubmit={handleSubmit} className='space-y-8'>
                  <div>
                    ...
                    <input
                      ...
                      required
                      readOnly={!!content?.guestData?.name}
                      className={`w-full px-6 py-4 rounded-2xl border-2 border-amber-100 focus:border-amber-400 focus:outline-none transition-all duration-300 ${
                        content?.guestData?.name ? 'bg-amber-50 cursor-not-allowed' : ''
                      }`}
                  ...
                  <motion.button
                    ...
                    type='submit'
                    disabled={submitting}
                    className='w-full bg-gradient-to-r from-amber-500 to-pink-500 text-white font-bold py-6 px-8 rounded-2xl text-xl shadow-xl hover:shadow-2xl transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed'
                  >
                    <div className='flex items-center justify-center gap-3'>
