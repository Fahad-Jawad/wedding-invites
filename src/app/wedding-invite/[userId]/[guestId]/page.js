...
export default function PreviewPageWithGuest() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch template data
        const templateResponse = await fetch(`/api/template/preview/${params.userId}`);
        ...
        // Fetch guest data
        if (params.guestId) {
          const guestResponse = await fetch(`/api/guests/public/${params.guestId}`);
          if (guestResponse.ok) {
            const guestResult = await guestResponse.json();
            setGuestData(guestResult.guest);
          }
        }
  ...
  if (templateData) {
    const enhancedContent = { ...templateData.content, guestData };

    switch (templateData.templateNumber) {
      case 1:
        return <Template1Preview content={enhancedContent} />;
      case 2:
        return <Template2Preview content={enhancedContent} />;
      case 3:
        return <Template3Preview content={enhancedContent} />;
      default:
        return (
